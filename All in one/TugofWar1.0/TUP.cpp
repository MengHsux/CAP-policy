				...
				for(int k=0;k<NumberofActions;k++)
				{
					for(int ps=0;ps<NumberofActions;ps++)
					{
						...
						for(int m=0;m<AgentNumber;m++)
						{
							...
							{
							case 0:
							  INDvalue[i][j][k][ps][qs].q[m]=abs(Qtable[m][CurrentPos][i][j][k][ps][qs]);
							  break;
						    case 1:
							  INDvalue[i][j][k][ps][qs].q[m]=abs(Qtable[m][CurrentPos][j][j][k][ps][qs]);
							  break;
						    case 2:
							  INDvalue[i][j][k][ps][qs].q[m]=abs(Qtable[m][CurrentPos][k][j][k][ps][qs]);
							  break;
							case 3:
							  INDvalue[i][j][k][ps][qs].q[m]=abs(Qtable[m][CurrentPos][ps][qs][i][j][k]);
							  break;
							case 4:
							  INDvalue[i][j][k][ps][qs].q[m]=abs(Qtable[m][CurrentPos][ps][qs][j][j][k]);
							  break;
							}

							INDvalue[i][j][k][ps][qs].axioms+=INDvalue[i][j][k][ps][qs].q[m]; //这个axioms没有任何作用
